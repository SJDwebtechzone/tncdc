...
import { Plus, Handshake, Trash2, X, Search } from "lucide-react";
import { useSelector, useDispatch } from 'react-redux';
import { addPartner, deletePartner } from '@/store/websiteSlice';
    ...
    return (
        <div className="space-y-6 relative">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h1 className="text-xl font-bold text-gray-800 font-sans">Manage Partners</h1>
                <Button
                                ...
                                <TableRow>
                                    <TableCell colSpan={5} className="py-24 text-center">
                                        <div className="flex flex-col items-center justify-center gap-4 text-gray-400">
                                            ...
                                            <p className="font-bold text-gray-400 italic">No partners found</p>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ) : (
                    ...
                    <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl p-6 relative">
                        ...
                        <form onSubmit={handleSave} className="space-y-4">
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-gray-600 uppercase">Partner Name <span className="text-red-500">*</span></label>
                                <Input required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="e.g. Google" className="h-11 rounded-xl bg-gray-50/50" />
                            </div>
                            <div className="space-y-2">
                            ...
                            <div className="flex justify-end gap-3 mt-8">
                                ...
                                <Button type="submit" className="bg-[#0f4c3a] hover:bg-[#0d3f30] text-white font-bold h-11 px-10 rounded-xl shadow-lg transition-all">Add Partner</Button>
                            </div>
                        </form>
                    </div>
                </div>
